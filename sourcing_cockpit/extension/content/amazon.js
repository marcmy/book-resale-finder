(async () => {
  ...
  panel.innerHTML = `
    ...
    <div class="sc-line">
      <label>Price $<input id="sc-fee-price" type="number" min="0" step="0.01"></label>
  ...
  panel.querySelector("#sc-amz-keepa").addEventListener("click", () => {
    window.open(`https://keepa.com/#!product/1-${asin}`, "_blank", "noopener");
  });
  ...
  bookmarkButton.addEventListener("click", async () => {
    ...
    if (map[asin]) {
      ...
      map[asin] = { asin, title, url: location.href, savedAt: Date.now() };
  ...
  panel.querySelector("#sc-note").addEventListener("input", e => {
    ...
    noteTimer = setTimeout(async () => {
      ...
      map[asin] = { text: e.target.value, updatedAt: Date.now(), title };
  ...
  panel.querySelector("#sc-fee").addEventListener("click", async () => {
    ...
    output.textContent = response?.ok
      ? `fees ${response.result?.totalFees?.amount ?? "?"} ${response.result?.totalFees?.currency || ""}`
      : (response?.error || "fee lookup failed");
  });

  const status = panel.querySelector("#sc-amz-status");
  try {
    const response = await chrome.runtime.sendMessage({ type: "CHECK_ELIGIBILITY", asin });
